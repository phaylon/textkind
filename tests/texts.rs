...
macro_rules! text_tests {
    ($name:ident: $dynamic:ty) => {
        ...
        mod $name {
            ...
            #[test]
            fn try_modify_cloned_dynamic_new() {

                let text = Test::try_from_str("foo").unwrap();

                let changed = text
                    .try_modify_cloned(|value| {
                        Modified::New(Dynamic::from_string(value.repeat(2)))
                    })
                    .expect("valid modification");
                assert_eq!(changed.as_str(), "foofoo");
                assert_eq!(text.as_str(), "foo");

                let error = text
                    .try_modify_cloned(|_value| {
                        Modified::New(Dynamic::from_string(String::new()))
                    })
                    .err()
                    .expect("invalid modification");
                assert!(format!("{:?}", error).contains("NotEmpty"));
                assert_eq!(text.as_str(), "foo");
            }

            #[test]
            fn try_modify_cloned_dynamic_sub() {

                let text = Test::try_from_str("foo").unwrap();

                let changed = text
                    .try_modify_cloned(|value| Modified::Sub(&value[..1]))
                    .expect("valid modification");
                assert_eq!(changed.as_str(), "f");
                assert_eq!(text.as_str(), "foo");

                let error = text
                    .try_modify_cloned(|value| Modified::Sub(&value[..0]))
                    .err()
                    .expect("invalid modification");
                assert!(format!("{:?}", error).contains("NotEmpty"));
                assert_eq!(text.as_str(), "foo");
            ...
            #[test]
            fn try_modify_cloned_dynamic_sub_static() {

                let text = Test::try_from_str("foo").unwrap();

                let changed = text
                    .try_modify_cloned(|_value| Modified::Sub("bar"))
                    .expect("valid modification");
                assert_eq!(changed.as_str(), "bar");
                assert_eq!(text.as_str(), "foo");
            ...
            #[test]
            fn try_modify_cloned_static_new() {
            ...
            #[test]
            fn try_modify_cloned_static_sub() {
